...
def add_section(uid: int, pr_name: str, title: str, paragraphs: list):
    section = Document(documentclass='subfiles', document_options=[NoEscape("../main.tex")])
    ...
    if os.path.exists(f"./UserFiles/{uid}/{pr_name}/sections"):
        with open(f"./UserFiles/{uid}/{pr_name}/jsons/sectionlist.json", 'r') as f:
            sectionlist = json.load(f)
    else:
        os.mkdir(f"./UserFiles/{uid}/{pr_name}/sections")
        sectionlist = []

    section.generate_tex(f"./UserFiles/{uid}/{pr_name}/sections/{len(sectionlist)}")
    sectionlist.append(title)

    with open(f"./UserFiles/{uid}/{pr_name}/jsons/sectionlist.json", 'w') as f:
        json.dump(sectionlist, f)


def build_document(uid: int, pr_name: str):
    with open(f"./UserFiles/{uid}/{pr_name}/jsons/config.json", 'r') as f:
        params = json.load(f)
    fontsize = params['fontsize']
    margin = params['indent']
    project = Document(documentclass='extarticle', fontenc='T1, T2A', document_options=f'{fontsize}pt')
    project.packages.append(Package('extsizes'))
    project.packages.append(Package('geometry', options=f'a4paper,margin={margin}mm'))
    project.packages.append(Package('indentfirst'))  # добавляет отступ у первого параграфа раздела
    ...
    n = len(sectionlist)
    for i in range(n):
        project.append(Command(command='subfile', arguments=NoEscape(f"./sections/{i}.tex")))

    project.generate_tex(f"./UserFiles/{uid}/{pr_name}/{pr_name}")
    project.generate_pdf(f"./UserFiles/{uid}/{pr_name}/{pr_name}", clean_tex=False)
...
if __name__ == '__main__':
    ...
    add_section(user_id, project_name, 'Subfile moment', [
    ...
    add_section(user_id, project_name, 'Subfile moment sequel', [
